-- script-estructura.sql
-- Define la estructura de las tablas

-- Nota 1 mayo 2026:
-- Se eliminó la tabla Reporte_Operacion y se ajustó la relación
-- para que Reporte dependa de Alerta en lugar de Operacion.
-- Se añadio atributos en usuario como contrasena para hacer una correcta autenticacion del login.
-- Se añadieron nuevas tablas para dar roles a los usuarios, en usuarios_tablas.sql


-- Nota 15 mayo 2026:
-- Se modificó la entidad Operacion para relacionarla directamente con Contrato
-- en lugar de Cliente y Producto, debido a que las operaciones pertenecen a
-- contratos específicos de apertura de crédito.
-- Se agregó el atributo Canal en Operacion para registrar el medio por el cual
-- se realizó la operación.
-- Se actualizó la entidad Contrato agregando la relación con Producto para
-- representar el tipo de servicio financiero contratado.
-- Se rediseñó la entidad Documento para asociarla a Contrato y Usuario,
-- permitiendo registrar documentación validada del expediente del cliente.
-- Se añadieron los atributos Ruta_Archivo, Fecha_Carga y Fecha_Validacion
-- para manejar archivos PDF y control de validaciones.
-- Se eliminó la relación entre Reporte y Alerta, dejando Reporte como una
-- entidad independiente para reportes regulatorios generados por el sistema.
-- Se modifico el token de seguimiento de la alerta de buzon por un hash para tener mayor seguridad
-- Se añadio una tabla intermedia entra caso y alerta, quitando id_alerta de Caso para ponerlo en "Caso_Alerta"
...
CREATE TABLE public."Alerta_Buzon" (
    ...
    Ruta_Evidencia TEXT,
    Hash_Seguimiento VARCHAR(64) NOT NULL
);
...
CREATE TABLE public."Caso_Alerta" (
    ID_Caso INTEGER NOT NULL,
    ID_Alerta INTEGER NOT NULL
);
...
ALTER TABLE public."Caso_Alerta"
ADD CONSTRAINT pk_caso_alerta PRIMARY KEY (ID_Caso, ID_Alerta);
...
ALTER TABLE public."Caso_Alerta"
ADD CONSTRAINT fk_caso_alerta_caso
FOREIGN KEY (ID_Caso) REFERENCES public."Caso"(ID_Caso);

ALTER TABLE public."Caso_Alerta"
ADD CONSTRAINT fk_caso_alerta_alerta
FOREIGN KEY (ID_Alerta) REFERENCES public."Alerta"(ID_Alerta);
