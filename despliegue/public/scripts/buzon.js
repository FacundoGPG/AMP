    ...
    row.addEventListener("click", () => {
      ...
      if (fileSpan && fileLink) {
        if (reporte.ruta_evidencia) {
          const nombreConToken = reporte.ruta_evidencia.split("/").pop().split("?")[0]; // quita token JWT
          const nombreArchivo = nombreConToken.replace(/^\d+_/, ""); // quita prefijo timestamp
          fileSpan.textContent = nombreArchivo;
          fileLink.href = `/get_private_file/${nombreConToken}`;
